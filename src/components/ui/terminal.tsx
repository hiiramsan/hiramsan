...
function tokenizeExperienceLine(text: string): Token[] | null {
  const match = text.match(
    /^(\d{2}\/\d{4}\s*-\s*\d{2}\/\d{4})(:\s*)(.+?)(\s*@\s*)(.+)$/,
  );
  if (!match) return null;

  return [
    { type: "date", value: match[1] },
    { type: "default", value: match[2] },
    { type: "role", value: match[3] },
    { type: "at", value: match[4] },
    { type: "company", value: match[5] },
  ];
}

const tokenColors: Record<TokenType, string> = {
  command: "text-emerald-400",
  company: "text-fuchsia-300",
...
function SyntaxHighlightedText({ text }: { text: string }) {
  const tokens = tokenizeExperienceLine(text) ?? tokenizeBash(text);
...
export function Terminal({
  ...
  const { down, up, unlock } = useAudio(enableSound);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setPhase("awaiting"), initialDelay);
  ...
  return (
    <div
      className={cn(
        "w-full max-w-xl font-mono text-xs",
        className,
      )}
    >
        ...
        >
          ...
          {phase === "awaiting" && (
            <div className="leading-relaxed whitespace-pre-wrap">
              ...
              <span
                className={cn(
                  "ml-2 inline-block h-4 w-2 bg-neutral-300 align-middle transition-opacity duration-100",
